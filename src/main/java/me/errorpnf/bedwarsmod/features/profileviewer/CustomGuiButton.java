...
public class CustomGuiButton {
    ...
    public void drawButton(float guiLeft, float guiTop, float x, float y, float width, float height, float mouseX, float mouseY, String text, int button, FontRenderer fontRenderer, float scaledX, float scaledY, float scaledWidth, float scaledHeight) {
        boolean isHovered = isMouseOver(mouseX, mouseY, scaledX, scaledY, scaledWidth, scaledHeight);

        // Bind the appropriate texture based on hover state
        if (isHovered) {
